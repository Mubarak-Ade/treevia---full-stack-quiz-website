...
import { useFetchCategories, useFetchRandomQuiz } from '@/modules/quiz/controllers/quiz-api.controller'
		...
		<div className='flex flex-col items-center p-5 justify-center'>
			...
			<div className="mt-5 p-5">
				...
				<ul className='grid grid-cols-3 gap-10'>
					{quiz.data.map((q) => (
						<QuizCard updatedAt={q.updatedAt} title={q.title} key={q._id} _id={q._id} difficulty={q.difficulty} questionCount={q.questionCount} timeLimitPerQuestion={q.timeLimitPerQuestion} />
					))}
				</ul>
			</div>
		</div>
